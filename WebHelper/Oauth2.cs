...
namespace WebHelper
{
    ...
    public class Oauth2
    {
        ...
        public string CodeGetOpenid(string Appid, string Appsecret, string Code)
        {
            ...
            string ReText = HttpClient.WebRequestPostOrGet(url, "");//post/get方法获取信息 
        ...
        public string GetUserInfo(string Appid, string Appsecret, string Code)
        {
            ...
            string ReText = HttpClient.WebRequestPostOrGet(url, "");//post/get方法获取信息
            Dictionary<string, object> DicText = (Dictionary<string, object>)Jss.DeserializeObject(ReText);
            return HttpClient.WebRequestPostOrGet("https://api.weixin.qq.com/sns/userinfo?access_token=" + DicText["access_token"] + "&openid=" + DicText["openid"] + "&lang=zh_CN", "");
        }
    }
}
